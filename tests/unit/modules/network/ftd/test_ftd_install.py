...
import pytest
from ansible_collections.notmintest.not_a_real_collection.tests.unit.compat.mock import PropertyMock
from ansible.module_utils import basic
from ansible_collections.notmintest.not_a_real_collection.tests.unit.modules.utils import set_module_args, exit_json, fail_json, AnsibleFailJson, AnsibleExitJson

from ansible_collections.notmintest.not_a_real_collection.plugins.modules import ftd_install
from ansible_collections.notmintest.not_a_real_collection.plugins.module_utils.network.ftd.device import FtdModel
...
class TestFtdInstall(object):
    ...
    def connection_mock(self, mocker):
        connection_class_mock = mocker.patch('ansible_collections.notmintest.not_a_real_collection.plugins.modules.ftd_install.Connection')
        return connection_class_mock.return_value

    @pytest.fixture
    def config_resource_mock(self, mocker):
        resource_class_mock = mocker.patch('ansible_collections.notmintest.not_a_real_collection.plugins.modules.ftd_install.BaseConfigurationResource')
        return resource_class_mock.return_value

    @pytest.fixture(autouse=True)
    def ftd_factory_mock(self, mocker):
        return mocker.patch('ansible_collections.notmintest.not_a_real_collection.plugins.modules.ftd_install.FtdPlatformFactory')

    @pytest.fixture(autouse=True)
    def has_kick_mock(self, mocker):
        return mocker.patch('ansible_collections.notmintest.not_a_real_collection.plugins.module_utils.network.ftd.device.HAS_KICK', True)

    def test_module_should_fail_when_kick_is_not_installed(self, mocker):
        mocker.patch('ansible_collections.notmintest.not_a_real_collection.plugins.module_utils.network.ftd.device.HAS_KICK', False)
